...
\subsection{Preparación entorno}

\subsubsection{Instalación con conda}
Antes de la ejecución del programa, será necesario instalar algunos requisitos. Para ello correr los siguientes comandos en consola:
...
\subsubsection{Descarga modelos}
Previo a la ejecución del programa, deben descargarse los pesos de los tres modelos que se evalúan.

Para ello, deben descargarse los archivos desde la plataforma ''HuggingFace'' y colocarse dentro de una carpeta \texttt{./models/} . \textbf{IMPORTANTE: Los archivos que se descarguen y sean tipo .bin deberán renombrarse con extensión .pth. Deben respetarse los nombres ddcolor\_comic.pth, ddcolor\_realistic.pth y ddcolor\_artistic.pth, así como el nombre de la carpeta que los incluye ''models'' que debe estar en la raíz.}

\begin{itemize}
	\item \texttt{ddcolor\_comic.pth}: modelo fine-tuned sobre imágenes de cómic. 
	Enlace: \url{https://huggingface.co/Aleparqui/PID_proyect/blob/main/net_g_latest.pth}
	\item \texttt{ddcolor\_realistic.pth}: modelo oficial de ModelScope (realista).
	Enlace: \url{https://huggingface.co/piddnad/ddcolor_modelscope/blob/main/pytorch_model.bin}
	\item \texttt{ddcolor\_artistic.pth}: modelo artístico de ModelScope.
	Enlace:\url{
		https://huggingface.co/piddnad/ddcolor_artistic/blob/main/pytorch_model.bin}
\end{itemize}

\paragraph{ • Ejemplo descarga ddcolor\_comic} 

(Hacer esto con cada uno de los modelos.)


\paragraph{PASO 1:} Entrar en el enlace suministrado, en este caso: \url{https://huggingface.co/Aleparqui/PID_proyect/blob/main/net_g_latest.pth}

\paragraph{PASO 2:} Darle al boton ''Download'' colocado arriba a la izquierda. Ver imágen.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{../assets/images/captura_hf.jpg}
	\caption{Captura sacada de Hugging Face para descargar el modelo}
	\label{fig:captura_hf}
\end{figure}

\paragraph{PASO 3:} Renombrar correctamente el archivo y añadirlo dentro de la carpeta DDColor/models.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{../assets/images/captura_explorador.jpg}
	\caption{Captura del explorador de archivos}
	\label{fig:captura_explorador}
\end{figure}

\subsection{Puesta en marcha}

Una vez cumplidos los requisitos y descargado los tres modelos, la aplicación se lanza ejecutando:

\begin{verbatim}
	python ddcolor_desktop.py
\end{verbatim}

\textbf{IMPORTANTE: } Al iniciarse, la aplicación carga los modelos en segundo plano e indica su estado en la esquina superior derecha de la ventana. Esperar a que carguen y que aparezca el mensaje \textit{Listo} en verde.
...
Los pesos del modelo fine-tuned se han publicado en el repositorio público de Hugging Face del proyecto:

\begin{center}
  \url{https://huggingface.co/Aleparqui/PID_proyect}
\end{center}

Hugging Face es una plataforma ampliamente utilizada en la comunidad de inteligencia artificial para alojar y compartir modelos, datasets y aplicaciones. Permite almacenar ficheros de pesos de forma gratuita con control de versiones basado en Git, facilitando tanto la descarga programática como la colaboración entre equipos.

\subsubsection{Estructura del repositorio}

El repositorio está registrado como un modelo de tipo \textit{Image-to-Image} con las etiquetas \texttt{image-colorization}, \texttt{ddcolor} y \texttt{comic-books}. Contiene:

\begin{itemize}
  \item El fichero de pesos \texttt{net\_g\_latest.pth}, generado por BasicSR al finalizar el entrenamiento. Almacena el estado del generador en la última iteración completada.
  \item Una tarjeta de modelo (\textit{Model Card}) en formato Markdown con la descripción completa del modelo: arquitectura base, dataset empleado, hiperparámetros de entrenamiento y ejemplos de inferencia.
\end{itemize}

\subsubsection{Proceso de subida}

La subida se realizó mediante la herramienta de línea de comandos \texttt{huggingface\_hub}. Los pasos seguidos fueron los siguientes:

\begin{enumerate}
  \item \textbf{Creación de la cuenta y el repositorio} en \url{https://huggingface.co}, seleccionando el tipo \textit{Model} y activando la visibilidad pública.

  \item \textbf{Autenticación local} mediante el token de acceso generado en la configuración de la cuenta:
  \begin{verbatim}
  huggingface-cli login
  \end{verbatim}

  \item \textbf{Subida del fichero de pesos}:
  \begin{verbatim}
  huggingface-cli upload Aleparqui/PID_proyect \
      ./models/net_g_latest.pth net_g_latest.pth
  \end{verbatim}

  \item \textbf{Redacción de la Model Card} directamente desde la interfaz web del repositorio, documentando el modelo base empleado (\texttt{piddnad/ddcolor\_modelscope}), el dataset de entrenamiento, los hiperparámetros principales y las limitaciones conocidas.
\end{enumerate}
...
En los tres casos, el fichero descargado debe renombrarse o referenciarse como \texttt{ddcolor\_comic.pth} para que la aplicación \textit{DDColor Lab} lo detecte correctamente, tal como se indica en la sección de manual de usuario.
