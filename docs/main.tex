...
Este decodificador emplea una arquitectura basada en la arquitectura de un \textit{transformer}, es decir, empleando mecanismos de atención, para refinar \textit{colour queries}.
...
Los mecanismos de atención tienen tres características principales \cite{IBMAttention}:
...
\begin{itemize}
  ...
  \item{\textbf{Autoatención}: Las \textit{queries} y las \textit{keys} proceden de la misma fuente. En DDColor, se emplea la \textit{multi-head attention}, en la que cada dimensión de la entrada se procesa por separado \cite{Attention}.}
\end{itemize}

Los mecanismos de atención son la base de los transformers, definidos en \cite{Attention}.

Éstos siguen también un patrón \textit{encoder-decoder}:
...
Así, las operaciones realizadas en cada CDB son las siguientes:
...
\begin{itemize}
  ...
  \item{\href{https://pypi.org/project/onnx/}{ONNX}: Librería para gestionar redes neuronales en formato ONNX (\textit{Open Neural Network eXchange}).}
\end{itemize}
...
\texttt{BasicSR} es un \textit{framework} que contiene múltiples utilidades empleadas por el modelo DDColor. Además, como parte del repositorio DDColor, se extiende dicho framework incluyendo, entre otros, una implementación del propio DDColor.

Este \textit{framework} está dividido en múltiples módulos:
...
El resultado de esta capa es después pasado por una función de activación ReLU, y posteriormente por la capa PixelShuffle en sí.
...
\begin{enumerate}
  ...
  \item{Capa BatchNorm2D \cite{TorchBatchNorm2D} (normalización) sobre el resultado almacenado en el \textit{hook}.}
...
\begin{enumerate}
  ...
  \item{\textbf{Opcionalmente}, una capa de activación ReLU.}
...
\texttt{SelfAttentionLayer} y \texttt{CrossAttentionLayer} son implementaciones de capas de autoatención y atención cruzada, respectivamente \cite{Attention}.

En cuanto a \texttt{FFNLayer}, éste implementa como módulo de PyTorch la arquitectura explicada en la sección del planteamiento teórico correspondiente a la red feed-forward encontrada al final de los CDBs.
...
\begin{itemize}
  ...
  \item{Tras cada capa lineal, se aplica una función de activación ReLU.}
\end{itemize}
...
Como se indica al principio de la clase en un comentario, esta es una versión ligeramente modificada de la implementación de la arquitectura de \textit{transformers} de PyTorch.
